...
public class HttpRedirector {
    ...
    /**
     * Http port, request will be redirected to
     */
    ...
    public final void redirectSSL(ProtocolInfo protocolInfo) throws IOException{
        String host = replacePort(parseHost(protocolInfo.byteBuffer));
        if (host == null){
            host = ipAddress + ":" +
                    redirectPort;
        }
    ...
    private String replacePort(String host) {
        ...
        if (delim != -1) {
            return host.substring(0, delim + 1) + redirectPort;
        }
        
        return host + ':' + redirectPort;
    }
}
